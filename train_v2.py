...
parser.add_argument('--fold', type=int, nargs='?', default=0, help="train for this fold.")
parser.add_argument('--p', type=str, nargs='?', default=None, help="")
    ...
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze) in enumerate(train_loader):
        try:
            with open(log_file_path, "a") as logfile:
                ...
                logger = f'{formatted_now} (train): fold[{fold}] epoch[{epoch+1}/{epochs}] iter[{i+1}/{len(train_loader)}]\tTime {batch_time.val:.3f} ({batch_time.avg:.3f})\tData {data_time.val:.3f} ({data_time.avg:.3f})\tLoss {losses.val:.4f} ({losses.avg:.4f})\t'
