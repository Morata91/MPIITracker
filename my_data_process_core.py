...
class norm:  
    ...
    def GetImage(self, image):
        print(f'Wmat{self.W_mat}, imsize{self.imsize}')
        print(f'shape{image.shape}')
        # self.im = cv2.warpPerspective(image, self.W_mat, (int(self.imsize[0]*1.5), int(self.imsize[1]*1.1)))
        self.im = cv2.warpPerspective(image, self.W_mat, (1280, 720))
        print(f'self.im:{self.im.shape}')
        return self.im
