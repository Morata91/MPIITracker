...
CP_PATH = 'checkpoints/0125_1'
EVAL_PATH = 'evallog/0125_1'


pltalllist = np.zeros([15,50])
pltalllist[:,:] = np.nan
# pltalllist = np.zeros((15,50))
AVGPATH = os.path.join(EVAL_PATH, 'avg')
...
def main():
    ...
    for i, fold in enumerate(range(15)):
    ...
    # for fold in range(len(cp_fold_list)):
        ...
        evallogpath = os.path.join(EVAL_PATH, f"fold_"+f'{fold:02d}')
        ...
        checkpoint_path = os.path.join(CP_PATH,cp_fold_list[i])
        ...
        with open(os.path.join(evallogpath, 'eval.log'), 'a') as outfile:
            ...
            for epochs in folder: 
                ...
                model.load_state_dict(saved_state_dict)
                model.cuda()
                error = evaluation(test_loader, model, evallogpath, fold, idx_tensor_x, idx_tensor_y, epochs.split('.')[0])
                if not onlybest:
...
def evaluation(test_loader, model, evallogpath, fold, idx_tensor_x, idx_tensor_y, epoch):
    ...
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze, binned_x, binned_y, imFacePath, eyeGrid) in enumerate(test_loader):
        try:
            with open(log_file_path, "a") as logfile:
                ...
                # compute output
                with torch.no_grad():
                    x, y, pre_x1 = model(imFace, imEyeL, imEyeR, faceGrid, eyeGrid)
                ...
                logger = f'{formatted_now} (test): fold[{fold}] epoch[{epoch}] iter[{i+1}/{len(test_loader)}]\tTime {batch_time.val:.3f} ({batch_time.avg:.3f})\tLoss x:{Total_losses_x.val:.4f} ({Total_losses_x.avg:.4f})\ty:{Total_losses_y.val:.4f} ({Total_losses_y.avg:.4f})\tユークリッド誤差 {Euc_losses.val:.4f} ({Euc_losses.avg:.4f})'
