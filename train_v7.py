...
def main():
    ...
    for fold in range(starting_fold,starting_fold+1):
        ...
        with open(log_file_path, "a") as logfile:
            logfile.write(f'project:{cp_time} fold[{fold}] batchsize:{batch_size} base_lr:{base_lr} alpha:{alpha}\n')
            logfile.write(f'x: {binwidth_x}cm, {-1*x_max}~{x_max} {xbins_num}分割  y:{binwidth_y}cm, 0~{y_max} {ybins_num}分割')
...
def train(train_loader, model, criterion_CLE, criterion_MSE, softmax, idx_tensor_x, idx_tensor_y, optimizer, epoch, epochs, fold, log_path):
    ...
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze, binned_x, binned_y, name) in enumerate(train_loader):
        ...
        with open(log_file_path, "a") as logfile:
            ...
            ## output(binned)
            x, y , prex1= model(imFace, imEyeL, imEyeR, faceGrid)
...
def validate(val_loader, model, criterion_CLE, criterion_MSE, softmax, idx_tensor_x, idx_tensor_y, epoch, fold, log_file_path):
    ...
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze, binned_x, binned_y, name) in enumerate(val_loader):
        try:
            with open(log_file_path, "a") as logfile:
                # measure data loading time
                # imFace = imFace.cuda()
...
def get_fc_params(model):
    ...
    b = [model.eyesFC, model.faceFC, model.gridFC, model.fc_x, model.fc_y]
    for i in range(len(b)):
        for module_name, module in b[i].named_modules():
            for name, param in module.named_parameters():
                yield param
