...
parser.add_argument('--doLoad', type=str2bool, nargs='?', const=True, default=True, help="Start from scratch (do not load).")
...
LOG_PATH = '/workspace-cloud/koki.murata/MPIITracker/trainlog'
...
def main():
    ...
    labelpathlist = os.listdir("/workspace-cloud/koki.murata/MPIITracker/datasets/Label")
    labelpathlist.sort()
    impath = "/workspace-cloud/koki.murata/MPIITracker/datasets/Image"
    ...
    for fold in range(starting_fold,15):
        ...
        # Quick test
        # if doTest:
        validate(val_loader, model, criterion, starting_epoch, fold, log_file_path)
        # return
        ...
        for epoch in range(starting_epoch, epochs):
            ...
            # train for one epoch
            train(train_loader, model, criterion, optimizer, epoch, epochs, fold, log_file_path)

            # evaluate on validation set
            prec1 = validate(val_loader, model, criterion, epoch, fold, log_file_path)
...
def train(train_loader, model, criterion,optimizer, epoch, epochs, fold, log_file_path):
    global count
    ...
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze) in enumerate(train_loader):
        try:
            with open(log_file_path, "a") as logfile:
                ...
                logger = f'(train): fold[{fold}] epoch[{epoch+1}/{epochs}] iter[{i+1}/{len(train_loader)}]\tTime {batch_time.val:.3f} ({batch_time.avg:.3f})\tData {data_time.val:.3f} ({data_time.avg:.3f})\tLoss {losses.val:.4f} ({losses.avg:.4f})\t'
                logfile.write(logger+'\n')
                print(logger)
        except Exception as e:
            print(f'エラー: {e}')

def validate(val_loader, model, criterion, epoch, fold, log_file_path):
    global count_test
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    ...
    for i, (row, imFace, imEyeL, imEyeR, faceGrid, gaze) in enumerate(val_loader):
        try:
            with open(log_file_path, "a") as logfile:
                ...
                logger = f'(val): fold[{fold}] [{epoch}] iter[{i+1}/{len(val_loader)}]\tTime {batch_time.val:.3f} ({batch_time.avg:.3f})\tLoss {losses.val:.4f} ({losses.avg:.4f})\tError L2 {lossesLin.val:.4f} ({lossesLin.avg:.4f})'
                logfile.write(logger+'\n')
                print(logger)
        except Exception as e:
            print(f'エラー: {e}')
