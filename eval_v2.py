...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Gaze estimation using MPIITracker .')
...
def main():
    ...
    for fold in range(len(cp_fold_list)):
        ...
        if not os.path.exists(os.path.join(evalpath, f"fold_"+f'{fold:02d}')):
            os.makedirs(os.path.join(evalpath, f"fold_"+f'{fold:02d}'))

        # list all epochs for testing
        #以下編集
        checkpoint_path = os.path.join(CP_PATH,"fold_"+f'{fold:02d}')
        folder = os.listdir(checkpoint_path)
        folder.sort()#01,02,03,...,best
        print(folder)
        
        softmax = nn.Softmax(dim=1)
        with open(os.path.join(evalpath, os.path.join("fold_"+f'{fold:02d}', 'eval.log')), 'w') as outfile:
            ...
            for epochs in folder: 
                ...
                model.cuda()
                log_file_path = os.path.join(evalpath, 'log.log')
